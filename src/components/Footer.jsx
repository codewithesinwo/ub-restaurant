...
<h3 className="text-4xl font-bold mb-6 bg-gradient-to-r from-amber-400 via-orange-400 to-amber-500 bg-clip-text text-transparent">
UB Restaurant
</h3>

<p className="text-gray-400 text-lg leading-relaxed max-w-md mb-10">
Delivering fresh, fast, and flavorful meals across Lagos.
Order online or pay cash on delivery every time.
</p>
...
{ icon: FaWhatsapp, href: "https://wa.me/2348031234567", label: "WhatsApp" },
].map((social, index) => (
<a
key={index}
href={social.href}
...
{navLinks.map((link) => (
<li key={link.path}>
<Link to={link.path} className="text-gray-300 hover:text-white transition-colors duration-200 block text-[15px]">
{link.name}
</Link>
</li>
))}
...
{quickLinks.map((link) => (
<li key={link.name}>
<a href={link.href} className="text-gray-300 hover:text-white transition-colors duration-200 block text-[15px]">
{link.name}
</a>
</li>
))}
...
<a href="tel:+2348031234567" className="hover:text-amber-400 transition-colors">+234 803 123 4567</a>
</div>
</div>
...
<a href="mailto:hello@ubrestaurant.com" className="hover:text-amber-400 transition-colors">hello@ubrestaurant.com</a>
</div>
</div>
...
<p className="leading-tight">12 Adeola Odeku Street,<br />Victoria Island, Lagos</p>
</div>
</div>
